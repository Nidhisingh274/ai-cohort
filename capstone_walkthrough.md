...
One item was not completed: traces from the in-cluster pod did not reach the Langfuse dashboard, because the langfuse package was missing from requirements-docker.txt used to build the deployed image - diagnosed via kubectl exec, and the try/except around every Langfuse call meant this failed silently rather than breaking the chat. The fix is understood (add the dependency, rebuild) but a rebuild did not finish within this submission's time window on this machine; see observability_notes.md for the full diagnosis.
